...
function getSessionSnapshot() {
  return {
    ...
    calcInput: calcInput,
    voiceOn: voiceOn,
    startAchievementTotal: (sess && typeof sess.startAchievementTotal === 'number') ? sess.startAchievementTotal : undefined,
    startAchievementStageLv: (sess && typeof sess.startAchievementStageLv === 'number') ? sess.startAchievementStageLv : undefined
  };
}
...
function applySessionSnapshot(snapshot) {
  ...
  if (src.startAchievementTotal === undefined) delete sess.startAchievementTotal; else sess.startAchievementTotal = src.startAchievementTotal;
  if (src.startAchievementStageLv === undefined) delete sess.startAchievementStageLv; else sess.startAchievementStageLv = src.startAchievementStageLv;
  syncLegacyStateAliases();
