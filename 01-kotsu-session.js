...
function kStart() {
  ...
  setSessionFields({ sess:{queue:sh(weak).concat(sh(rest)).slice(0,Math.min(ps.length,20)),idx:0,results:[],streak:0,startTime:0,sessStartTime:Date.now(), _sessionEnding:false, _specialOver:false, _specialAnswerLocked:false, startAchievementTotal:startAchievementTotal, startAchievementStageLv:startAchievementStage ? startAchievementStage.lv : null}, sessMode:'kotsu' });
